...
% 2017.3.2 CTA3 is for the pre-twiddle radix operation, but currently we do
% not use this version. Please go to CTA2.
%--------------------------------------------------
                    ...
                    for t_n5 = 0:Nf_stage(5) -1
                        for t_n6 = 0:Nf_stage(6) -1
                            ...
                            if (m==1 || m==2)
                                ...
                                tw_base_pre = sum_addr_coeff(m-2);
                            end
                            ...
                            case 2
                            for t = 1:5
                                tw_N_exp(t) = Nf(6)*Nf(5)*Nf(4)*Nf(3)*(t-1)+Nf(6)*Nf(5)*Nf(4)*t_n3+Nf(6)*Nf(5)*t_n4+Nf(6)*t_n5+t_n6;
                            end
                            tw_coeff = exp(-1i*2*pi*t_n1*tw_N_exp/tw_base_pre);
                            case 3
                            for t = 1:5
                                tw_N_exp(t) = Nf(6)*Nf(5)*Nf(4)*(t-1)+Nf(6)*Nf(5)*t_n4+Nf(6)*t_n5+t_n6;
                            end
                            tw_coeff = exp(-1i*2*pi*t_n2*tw_N_exp/tw_base_pre);
                            case 4
                            for t = 1:5
                                tw_N_exp(t) = Nf(6)*Nf(5)*(t-1)+Nf(6)*t_n5+t_n6;
                            end
                            tw_coeff = exp(-1i*2*pi*t_n3*tw_N_exp/tw_base_pre);
                            case 5
                            for t = 1:5
                                tw_N_exp(t) = Nf(6)*(t-1)+t_n6;
                            end
                            tw_coeff = exp(-1i*2*pi*t_n4*tw_N_exp/tw_base_pre);
                            case 6
                            for t = 1:5
                                tw_N_exp(t) = t-1;
                            end
                            tw_coeff = exp(-1i*2*pi*t_n5*tw_N_exp/tw_base_pre);
                            otherwise
                            tw_N_exp = zeros(5,1);
                            tw_coeff = ones(5,1);
                            end
                            ...
                            fft_tw_out = fft_tw_CTA3(read_data_index(1:5), Nf(m), tw_coeff, is_first_stage );
