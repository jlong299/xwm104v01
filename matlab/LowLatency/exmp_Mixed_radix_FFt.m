...
for m=1:NumOfFactors
    ...
    for t_n2=0:Nf(2)-1
        for t_n3=0:Nf(3)-1
            ...
            fft_tw_out = fft_tw(read_data_index, Nf(m), [tw_N^((n(2)*Nf(3)+n(3))*0);tw_N^((n(2)*Nf(3)+n(3))*1);tw_N^((n(2)*Nf(3)+n(3))*2);0;0] );
    ...
    for t_k1=0:Nf(1)-1
        for t_n3=0:Nf(3)-1
            ...
            % radix-factor fft and twiddle
            fft_tw_out = fft_tw(read_data_index, Nf(m), [tw_N^(n(3)*0);tw_N^(n(3)*1);tw_N^(n(3)*2);tw_N^(n(3)*3);0] );
            % write data to each bank of another RAM
            for t = 1:NumOfBanks
    ...
    for t_k1=0:Nf(1)-1
        for t_k2=0:Nf(2)-1
            ...
            % radix-factor fft and twiddle
            fft_tw_out = fft_tw(read_data_index, Nf(m), [1;1;1;1;1] );
            % write data to each bank of another RAM
            for t = 1:NumOfBanks
