...
% RAM_0 = zeros(N_max,NumOfBanks);  % 5 banks
% RAM_1 = zeros(N_max,NumOfBanks);  % 5 banks
% current_RAM = 0;
% RAM_read = zeros(N_max,NumOfBanks);  % 5 banks
% RAM_write = zeros(N_max,NumOfBanks);  % 5 banks
%---------------------------------
                ...
                for t_n4 = 0:Nf_stage(4) -1
                    for t_n5 = 0:Nf_stage(5) -1
                        for t_n6 = 0:Nf_stage(6) -1
                            ...
                            for t = 1:NumOfBanks
                                if (t <= Nf(m))
                                read_data_index(t) = RAM_read(coeff_bank(m)*(t-1)+ coeff_bank(1)*t_n1+coeff_bank(2)*t_n2+coeff_bank(3)*t_n3+coeff_bank(4)*t_n4+coeff_bank(5)*t_n5 +1, read_bank_index(t) +1);
                                else
                                read_data_index(t) = 0;
                                end
                            end
                            ...
                            for t = 1:NumOfBanks
                                if (t <= Nf(m))
                                RAM_write(coeff_bank(m)*(t-1)+ coeff_bank(1)*t_n1+coeff_bank(2)*t_n2+coeff_bank(3)*t_n3+coeff_bank(4)*t_n4+coeff_bank(5)*t_n5 +1, read_bank_index(t) +1) = fft_tw_out(t);
                                end
                            end
                        end
                    end
