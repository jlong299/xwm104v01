...
module CTA_addr_source_p2 #(parameter
		wDataInOut = 12
	)
	(
	input clk,    
	...
	output logic [0:1][wDataInOut-1:0] addr_source_CTA 
);
...
begin
	if (!rst_n || !clr_n) begin
		...
		// if (1'b1) 
			...
			k0_x_twdl_dem1 <= k0_x_twdl_dem1 + (twdl_demontr[1]<<1);
		else
...
logic [wDataInOut-1:0]  addr_source_CTA_pre;
// logic [wDataInOut-1:0]  twdl_demontr_1x2;
always@(posedge clk)
begin
